...
public class FileExam2 {
	public static void main(String[] args) throws IOException  {
		...
		// File 객체생성은 파일의 존재여부와 관계없이 생성이 가능하다.
		System.out.println(file1);
		
		// 절대경로: root 디렉토리(폴더)로 부터 전체 경로를 표시해서 지정한다.
		// C:MyStudy\temp\aaa
		
		// 상대경로: 현재위치로부터 찾아가고자 하는 위치를 표시해서 지정한다.
		// .\..\..\aaa\a1 또는 ..\aaa 또는 temp\aaa
		
		// 절대주소방식 C:\\temp\\aaa\\a1.temp.txt
		...
		System.out.println("-------- File getCaonoicalPath() ------");
		...
		try {
			file4.createNewFile(); // IOException: 디렉토리 없을 때도 발생한다.
		} catch (IOException e) {
			e.printStackTrace();
		}
		file4.delete();
