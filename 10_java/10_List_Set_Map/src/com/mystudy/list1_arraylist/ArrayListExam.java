...
public class ArrayListExam {
	...
	public static void main(String[] args) {
		...
		list1.add(1); // int -> Integer 자동형변환 되어서 입력처리가된다.
		list1.add(5);
		list1.add(6);
		list1.add(7);
		...
		System.out.println("0번 인덱스에 100 추가후(add): " + list1.toString());

		// 수정: 지정된 위치에 데이터 수정(변경)
		list1.set(0, 999); // 인덱스 위치의 데이터 변경(수정)
		System.out.println("0번 인덱스에 999 설정(set): " + list1.toString());

		// 수정(set): 존재하지 않는 인덱스 접근시 오류 IndexOutOfBoundsException
		// list1.set(100, 999); // 인덱스 위치의 데이터 변경(수정)
		// System.out.println("100번 인덱스에 999 설정(set): " + list1.toString());

		// 삭제: 특정 위치의 데이터 삭제
		list1.remove(0); // 인덱스 위치 데이터 삭제
		System.out.println("0번 인덱스 삭제(remove) " + list1.toString());
		...
		// Collectnons에는 내림차순 기능은 없어서 sort() 후 reverse 해주기
...
//		printData(list4);
		...
		// 데이터가 있는지 여부 확인 ->//boolean 값이 넘어온다 
...
//			System.out.println("list2 삭제: " + list2.remove(0));
//		}
...
//		int size = list2.size();
//		for (int i = 0; i < size ; i++) {
//			System.out.println("list2 삭제: " + list2.remove(list2.size()-1)); <- 마지막 인덱스
//		}
//	
