package com.mystudy.socket04_echo_while;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStream;
import java.io.InputStreamReader;
import java.io.OutputStream;
import java.io.PrintWriter;
import java.net.ServerSocket;
import java.net.Socket;

public class TCPServerEchoWhile {
	public static void main(String[] args) {
		// 서버쪽
		// 클라이언트에서 보내는 메시지 받고 다시 보내기 반복처리
		// 수신(input)메시지 화면 출력 후 클라이언트로 전송(output)
		// 클라이언트에서 종료요청시 까지

		ServerSocket server = null; // 서버 생성
		BufferedReader br = null;
		PrintWriter pw = null;

		try {
			server = new ServerSocket(10000); // 강사pc: 192.168.0.200 
			System.out.println(">> 서버 시작");
			System.out.println(">> 클라이언트 접속 대기중");

			Socket socket = server.accept(); // 클라이언트에서 접속이되었다는 뜻
			System.out.println(">>> 클라이언트 접속됨");

			// 연결된 소켓과 데이터를 주고받기 위한 I/O 객체 생성
			InputStream is = socket.getInputStream();
			OutputStream os = socket.getOutputStream();

			// 바이트(byte) -> 문자(char) 처리계열로 변경 필터 적용
			InputStreamReader isr = new InputStreamReader(is);

			// 버퍼(buffer) 사용으로 편리하게 라인단위 입출력
			// 버퍼는 리더를 거쳐야만 사용할 수 있다.
			br = new BufferedReader(isr);
			pw = new PrintWriter(os);

			while (true) {
				// 읽고
				String msg = br.readLine();
				System.out.println("받은메시지: " + msg);

				// 클라이언트에서 작업종료하면(exit)하면 종료 처리
				if (msg == null || "eixt".equalsIgnoreCase(msg)) {
					System.out.println("서버를 중단합니다.");
					break;
				}

				// 쓰기
				pw.println(msg);
				pw.flush(); // 버퍼데이터 출력처리

			}

		} catch (IOException e) {
			e.printStackTrace();
			return;
		} finally {
			if (br != null) {
				try {
					br.close();
				} catch (IOException e) {
					e.printStackTrace();
				}
				pw.close();
			}
		}

		try {
			server.close();
		} catch (IOException e) {
			e.printStackTrace();
		}

		System.out.println(">>> 서버 종료");

	}
}
