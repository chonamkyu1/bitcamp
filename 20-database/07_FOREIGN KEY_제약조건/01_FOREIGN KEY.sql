...
테이블 생성시 제약조건 설정
 ...
 - 외래키 (FROEIGN KEY) 지정으로 관계 설정 
 - 형태: 컬럼정의 REFERENCES 대상테이블 (대상컬럼)
*/
SELECT * FROM DEPT;
-- 컬럼레벨에서 제약조건(외래키) 설정
CREATE TABLE EMP01 (
...
--ORA-02291: integrity constraint (MADANG.SYS_C007048) violated - parent key not found
INSERT INTO EMP01 (EMPNO, ENAME, JOB, DEPTNO)
...
*/
-- EMP01 테이블의 PRIMARY KEY 삭제: SYS_C007047
...
-----------------------------------
-- 제약조건 설정시 데이터 상태에 따라 적용여부 결정된다.
-- 중복데이터 있는 경우 PRIMARY KEY, UNIQUE 설정 안된다.
-- NOT NULL 설정하려는 컬럼에 NULL 데이터가 하나라도 있으면 안된다.
-- (해결방법) 중복데이터를 삭제하거나 수정처리 UNIQUE하게
--           NULL인 데이터 찾아서 수정처리 (NULL 아닌 값으로)
ALTER TABLE EMP01 ADD CONSTRAINT EMP01_EMPNO_PK PRIMARY KEY (EMPNO);

-- EMP02 외래키(FOREIGN KEY) 이름 변경 처리
ALTER TABLE EMP02 DROP CONSTRAINT SYS_C007051;
...
alter table EMP02 disable  constraint EMP02_DEPTNO_FK; --비활성화 상태
