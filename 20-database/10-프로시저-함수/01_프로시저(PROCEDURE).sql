...
DECLARE
    ...
    V_PRICE NUMBER(8);
BEGIN
    -- SELECT ~ INTO ~ FROM: DB데이터 선택하고  INTO절에 변수 저장한다,
    -- 1개의 데이터만 처리가능하다.
...
CREATE OR REPLACE PROCEDURE GET_BOOKINFO (
    IN_BOOKID IN NUMBER, -- 매개변수 선언시 타입만 지정한다.
    OUT_BOOKNAME OUT VARCHAR2,
    OUT_PUBLISHER OUT VARCHAR2,
    OUT_PRICE OUT NUMBER
) AS
    -- %TYPE 사용: 테이블.컬럼명%TYPE
    -- 테이블 컬럼과 동일한 타입으로 선언(변경시에도 적용된다.)
    V_BOOKID BOOK.BOOKID%TYPE; --북테이블에있는 북아이디 타입 그대로 쓴다는말
...
BEGIN
    ...
    -- 데이터를 호출한 곳으로 전달하기 위해 OUT 유형 변수에 저장한다.
...
AS
    V_CUSTID CUSTOMER.CUSTID%TYPE;   -- 변수선언
