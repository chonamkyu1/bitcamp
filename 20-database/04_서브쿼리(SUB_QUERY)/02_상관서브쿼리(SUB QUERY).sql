/* 상호연관 서브쿼리(상관서브쿼리)
-- 메인쿼리의 값을 서브쿼리가 사용하고, 서브쿼리의 결과값을 메이쿼리가 사용한다.
...
SELECT *
 FROM BOOK B
 WHERE NOT EXISTS (SELECT BOOKNAME FROM BOOK 
               WHERE B.BOOKNAME LIKE '%축구%') -- 축구가 안들어간 모든 책들        
;
...
SELECT *
 FROM CUSTOMER C
 WHERE EXISTS (SELECT * FROM ORDERS -- SELECT 후에 아무거나 와도 상관없다.(EXIST문)
