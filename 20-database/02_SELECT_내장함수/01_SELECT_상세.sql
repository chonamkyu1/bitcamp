...
;
SELECT * FROM BOOK
 WHERE NOT (PUBLIHSER = '굿스포츠' OR PUBLISHER = '대한미디어')
;
--======
-- IN : 안에 있냐? (OR문 단순화)
--(실습) 출판사 나무수, 대한미디어, 삼성당 에서 출판한 도서 목록
SELECT * FROM BOOK
...
SELECT * FROM BOOK
 ...
 ORDER BY PRICE
;
-- NOT BETWEEN 값1(부터) AND 값2(까지) : 값1 부터 값2 까지 형태로 사용
SELECT * FROM BOOK
...
SELECT * FROM BOOK
 WHERE BOOKNAME LIKE '_구의%' -- _위치에 반드시 1개 문자가 있어야함
;
------------------
--(실습) 책제목에 '구' 문자가 2번째에 있는 책 조회
SELECT * FROM BOOK
 WHERE BOOKNAME LIKE '_구%' -- _위치에 반드시 1개 문자가 있어야함
;
--(실습) 책제목에 '를' 문자가 3번째에 있는 책 조회
SELECT * FROM BOOK
 WHERE BOOKNAME LIKE '__를%' -- _위치에 반드시 1개 문자가 있어야함
;
----------------------------------------
--LIKE 사용시 동작 결과 확인
