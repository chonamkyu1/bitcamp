...
- 자동인덱스: PRIMARY KEY 선언시 또는 UNIQUE 제약조건 설정시 생성되는 인덱시
...
SELECT * FROM BOOK WHERE PUBLISHER = '굿스포츠'; -- F10 플랜확인
...
SELECT * FROM BOOK WHERE PRICE > 10000; -- 인덱스 적용 안함
SELECT PUBLISHER, PRICE FROM BOOK ORDER BY PUBLISHER, PRICE; 

-- WHERE 절에 PUBLISHER, PRICE 컬럼 동시 사용시 IX_BOOK2 인덱스 적용 여부
...
SELECT * FROM BOOK WHERE SUBSTR(PUBLISHER,1,2) = '대한'; -- 인덱스 적용안됨
SELECT * FROM BOOK WHERE UPPER(PUBLISHER)  = 'PEARSON'; -- 인덱스 적용안됨

-- 인덱스 삭제
DROP INDEX IX_BOOK;

/* 실습
마당서점 데이베이스에서 다음 SQL문을 수행하고
데이터베이스가 인덱스를 사용하는 과정을 확인하시오.
(1) 다음 SQL문을 수행해본다
    SELECT name FROM Customer WHERE name LIKE '박세리'ㅣ
(2) 실행 계획을 살펴본다. 실행 계획은 F10을 누른 후
    [계획 설명] 탭을 선택하면 표시된다.
(3) Customer 테이블에 name으로 인덱스(ix_customer_name)를 생성하시오.
    생성 후 (1)번의 질의를 다시 수행하고 실행 계획을 살펴보시오.
(4) 같은 질의에 대한 두 가지 실행 계획을 비교해보시오.
(5) (3)번에서 생성한 인덳를 삭제하시오.
*/
SELECT NAME FROM CUSTOMER WHERE NAME LIKE '박세리';
-- Customer 테이블에 name으로 인덱스(ix_customer_name)를 생성하시오.
CREATE INDEX ix_customer_name ON CUSTOMER(NAME);

SELECT NAME, ROWID FROM CUSTOMER CUSTOMER ORDER BY NAME ;
