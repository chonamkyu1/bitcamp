...
<%-- 전달받은 데이터를 DB에 저장(INSERT) 
	입력성공시: 목록페이지로 이동(list.jstp)
	입력실패(예외)시: 메시지 표시 후 이전화면(입력페이지) 이동
--%>
<%
	// 0. 한글처리
...
<%
	...
	// 3. SqlSession 객체를 사용해서 mapper의 insert SQL문 실행(VO데이터 전달)
	try {
...
<% 		
		...
		// 4. 페이지 이동(전환) 처리 - 예외발생한 경우
		System.out.println(">>[예외발생] 입력작업중 오류가 발생했습니다.");
		e.printStackTrace();
%>
	<script>
		alert("[예외발생] 입력중 예외가 발생하였습니다.\n" + "담당자(8282)에게 연락하세요.\n"
				+ "이전페이지로 이동합니다.");
		history.back();
	</script>
<% 		
